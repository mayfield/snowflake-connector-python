...
class ChunkTracker(object):
    """ Individual stat for each download.  Used to track progress and
    completion. """
    ...
    def __getitem__(self, rng):
        ...
        with self._lock:
            for record in self._records:
                if (start is None or not record.done or
                    start < record.finished) and \
                   (stop is None or stop > record.started):
                    ...
                    partial.accum *= partial.elapsed / record.elapsed
                    out.append(partial)
    ...
    def concurrency(self):
        with self._lock:
            ...
            last_finish = max(x.finished if x.done else now
                              for x in self._records)
            walltime = last_finish - first_start
            return sum(x.elapsed for x in self._records) / walltime
...
class SnowflakeChunkDownloader(object):
    ...
    def _fetch_chunk_worker(self, chunk):
        ...
        timeouts = (
            ...
            5  # how long to wait for initial equest data
        )
        ...
        with rest._use_requests_session() as session:
            ...
            with self._stats.track(length) as tracker:
                for x in resp.iter_content(2 ** 16):
                    buf.append(x.decode())
                    tracker.set_accum(resp.raw.tell())
                assert tracker.accum == tracker.length, (tracker.accum, tracker.length)  # XXX remove since this is not a real gaurentee
        buf.append(']')
